...
// Import our custom type checker
import { is } from '../config/fp-core/index.js';
...
const normalizeModule = (moduleExport, ctx) => {
  // Use our custom is function for type checking
  return is(Function)(moduleExport)
...
const isValidModule = (mod) =>
  is(Object)(mod) && // Must be an object
...
export function loadModule(ctx, moduleExport) {
  if (!ctx.app) {
    throw new Error(
      '[loadModule] Context object (ctx) must contain an initialized App instance (ctx.app).'
    );
  }
  if (!moduleExport) {
  ...
  try {
    ...
    throw new Error(
      `[loadModule] Failed to normalize module export. Original error: ${err.message}`
    );
  }
  ...
  if (!isValidModule(mod)) {
    ...
    const exportName =
      typeof moduleExport === 'function'
        ? moduleExport.name || '(anonymous function)'
        : '(object export)';
    ctx.logger?.error(
      `[loadModule] Invalid module structure for export: ${exportName}. Received:`,
      mod
    );
    throw new Error(
      `[ApolloOS] Invalid module structure: Module must return an object with a valid string 'id'. Received: ${JSON.stringify(mod)}`
    );
  }
...
export function composeModules(ctx, modulesToLoad = []) {
  ...
  const loadedModules = modulesToLoad.map((moduleExport, index) => {
    try {
      ...
      throw new Error(
        `Failed to load module at index ${index}. Aborting batch load. Error: ${err.message}`
      );
