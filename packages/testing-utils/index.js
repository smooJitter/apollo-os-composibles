...
export const createMockContext = (overrides = {}, appOverrides = {}) => {
  ...
  const mockApp = {
    ...
    getModule: function(id) { return this.registry[id]; },
    getModules: function() { return this.modules; }, // Return module definitions
    getAllRegisteredAssets: function() { return Object.values(this.registry); }, // Return registered assets
    getModels: function() { return this.getAllRegisteredAssets().reduce((acc, mod) => { if (mod.models) acc[mod.id] = mod.models; return acc; }, {}); },
    getTypeComposers: function() { return this.getAllRegisteredAssets().reduce((acc, mod) => { if (mod.typeComposers) acc[mod.id] = mod.typeComposers; return acc; }, {}); },
  ...
  const mockCtx = {
    ...
    enums: {
      roles: { // Example roles, adjust as needed
        ROLES: { USER: 'user', ADMIN: 'admin' },
        ROLE_VALUES: ['user', 'admin'],
      },
    },
    fp: { // Mock essential FP utils if modules use them directly
        R: {}, 
...
export const createMockModule = (id, assets = {}, lifecycle = {}) => {
  return (ctx) => ({
    ...
    onLoad: lifecycle.onLoad || function() {
    ...
    init: lifecycle.init || function() { ctx.logger?.debug(`[MockModule ${id}] init executed.`); },
    relations: lifecycle.relations || function() { ctx.logger?.debug(`[MockModule ${id}] relations executed.`); },
    hooks: lifecycle.hooks || function() { ctx.logger?.debug(`[MockModule ${id}] hooks executed.`); },
