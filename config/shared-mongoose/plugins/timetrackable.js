// plugins/timeline-trackable.js

module.exports = function timelineTrackablePlugin(schema, options = {}) {
    const {
      field = 'timeline',
      userField = 'actor',
      eventField = 'event',
    ...
    schema.methods.logEvent = function (eventKey, actorId = null, note = null) {
      if (enableEventTypes && !allowedEvents.includes(eventKey)) {
        throw new Error(`Invalid timeline event: "${eventKey}". Must be one of: ${allowedEvents.join(', ')}`);
      }
